...
<body>
    ...
    <div class="flex flex-col items-center justify-center mx-auto mt-20 gap-12 max-2-screen-lg">
        <form action="{{ route('profile.update') }}" method="post" enctype="multipart/form-data">
            ...
            <div class="h-[145px] flex-col justify-start items-start gap-[3px] flex">
                <div class="self-stretch h-[122px] flex-col justify-start items-start gap-[7px] flex">
                    <div class="justify-start items-center gap-2.5 inline-flex">
                        <div class="text-black text-opacity-50 text-xs font-normal font-['Roboto'] leading-3 tracking-tight">自己紹介文</div>
                    </div>
                    ...
                    <div class="w-[480px] h-px pt-px justify-center items-center inline-flex">
                        <div class="w-[480px] h-[0px] border border-black"></div>
                    </div>
                </div>
                <!-- バリデーションメッセージ -->
                @error('introduction')
                    <div class="h-5 flex-col justify-start items-center gap-2.5 flex">
                        <div class="text-red-500 font-bold text-xs font-['Roboto'] leading-tight tracking-wide">{{ $message }}</div>
                    </div>
                @enderror
            </div>

            <!-- アバター画像 -->
            <div class="mt-4 h-12 flex-col justify-start items-start gap-2.5 flex">
                <div class="self-stretch h-12 flex-col justify-start items-start gap-[3px] flex">
                    <div class="pr-[278px] mt-5 flex-col justify-center items-start gap-1 flex">
                        ...
                        <div class="self-stretch px-6 py-2 bg-black bg-opacity-10 rounded justify-center items-center inline-flex cursor-pointer hover:bg-gray-300 relative">
                            ...
                            <label for="avatar" class="cursor-pointer text-black text-opacity-75 text-sm font-normal font-['Roboto'] hover:bg-gray-300">画像ファイルを添付する</label>
                        </div>
                    </div>
                    <!-- ファイル名表示 -->
                    ...
                    <!-- バリデーションメッセージ -->
                    @error('avatar')
                        <div class="text-red-500 text-xs font-bold font-['Roboto'] leading-tight tracking-wide">{{ $message }}</div>
                    @enderror
                </div>
            </div>
            ...
            <!-- 確定ボタン -->
            <div class="px-20 py-4 mt-24 ml-12 bg-cyan-800 rounded justify-center items-center inline-flex cursor-pointer hover:bg-cyan-900">
                <button type="submit" class="text-white text-sm font-normal font-['Roboto']">自己紹介を確定する</button>
            </div>
        </form>
    </div>
    <!-- footer -->
